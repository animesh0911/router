...
impl Subgraph<Initial> {
    ...
    pub fn into_fed2_subgraph(self) -> Result<Self, FederationError> {
        ...
        let inner_schema = schema_as_fed2_subgraph(schema, &Version{major: 2, minor: 4})?;
...
fn schema_as_fed2_subgraph(
    ...
    let fed_spec = FEDERATION_VERSIONS.find(version)
        .ok_or_else(|| SingleFederationError::UnknownFederationLinkVersion{message: format!("Unknown federation spec version: {}", version)})?;
    ensure!(
    ...
    let imports: Vec<_> = fed_spec
        ...
        .iter()
        .map(|d| format!("@{}", d.name()))
        .chain(fed_spec.type_specs().iter().map(|t| t.name().to_string()))
        .collect();
    ...
    inner_schema
        ...
        .push(Component::new(Directive {
            ...
            arguments: vec![
                ...
                Node::new(ast::Argument {
                    name: LINK_DIRECTIVE_IMPORT_ARGUMENT_NAME,
                    value: Node::new(ast::Value::List(imports.iter().map(|s| s.into()).collect())),
                }),
            ],
        }));
    Ok(inner_schema)
